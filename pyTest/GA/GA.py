...
#         report_FinalScore[key] = new_FinalScore
    ...
    def printMetrics(self,test, real_codes, w):
        result = {}
        for reportName in test:
            ...
            for codeName in self.SC[reportName].keys():
                if self.SC[reportName][codeName] > 0 or self.SIM[reportName][codeName] > 0:
                    FinalScore[codeName] = w[0] * self.RIC[reportName][codeName] + w[1] * self.SC[reportName][codeName] + w[2] * \
                                           self.SIM[reportName][codeName] + w[3] * self.STC[reportName][codeName] + w[4] * \
...
def run(n,path,name_of_project):
    ...
    for i in range(handler.N_GENERATIONS):
        ...
        end_str = '100%'
        handler.process_bar(i / handler.N_GENERATIONS, start_str='', end_str=end_str, total_length=15)
        # 交叉和变异
    ...
    return_val=handler.printMetrics(handler.test, real_codes, [W1, W2, W3, W4, W5])
    f.close()
    print(time.asctime(time.localtime(time.time())))
    return return_val
...
if __name__ == "__main__":
    ...
    print(run(95,"D:\\个人\\作业\\大三（下）\\软工三\\backend-irblapp\\JSON-AspectJ","AspectJ"))
